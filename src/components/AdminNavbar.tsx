...
const AdminNavbar = () => {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${
      isScrolled ? 'bg-white/95 backdrop-blur-md shadow-sm py-2' : 'bg-dark py-4'
    }`}>
      <div className="container mx-auto px-4 md:px-6 flex justify-between items-center">
        ...
        <div className="hidden md:flex items-center space-x-8">
          {[
            ...
            { id: 'enquiries', label: 'Enquires', to: '/enquiries' },
            { id: 'logout', label: 'Logout', to: '/' }
          ].map((item) => (
            <Link 
              key={item.id}
              to={item.to}
              className={`relative px-2 py-1 text-lg font-medium ${
                isScrolled 
                  ...
                  : (activeLink === item.id || (item.to === '/menu' && isMenuPage) || (item.to === '/media' && isMediaPage))
                    ? 'text-emerald-300'
                    : 'text-dark hover:text-emerald-300'
