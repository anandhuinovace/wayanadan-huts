...
const TestimonialCard = ({ quote, author, place, state, rating, image, delay = "0s", highlight }: TestimonialProps) => (
  <div 
  ...
  >
    <div className="flex items-center mb-6">
      ...
      <div>
        <h4 className="text-lg font-bold text-gray-900 break-words">{author}</h4>
        <p className="text-gray-600 text-sm">{place}, {state}</p>
        <div className="flex mt-1">
          {[...Array(5)].map((_, i) => (
            <Star 
              key={i} 
              className={`w-4 h-4 flex-shrink-0 ${i < rating ? 'fill-yellow-400 text-yellow-400' : 'text-gray-300'}`} 
            />
          ))}
        </div>
      </div>
    ...
    <div className="relative flex-grow">
      ...
      <p className="text-gray-700 pl-8 text-lg leading-relaxed break-words">&ldquo;{quote}&rdquo;</p>
      {highlight && (
  ...
  useEffect(() => {
    const fetchTestimonials = async () => {
      try {
        ...
        const q = query(collection(db, "ratings"), where("status", "==", "approved"));
            ...
            <div className="mt-16 flex flex-wrap justify-center items-center gap-8 md:gap-16 scroll-animate" style={{ animationDelay: "0.4s" }}>
              <div className="text-center">
                ...
                <div className="flex justify-center">
                  {[...Array(5)].map((_, i) => (
                    <Star key={i} className={`w-5 h-5 ${i < Math.round(averageRating) ? 'fill-yellow-400 text-yellow-400' : 'text-gray-300'}`} />
                  ))}
                </div>
